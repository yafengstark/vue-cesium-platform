...
const store = new Vuex.Store({
    ...
    mutations: {
        SET_CESIUMINSTANCE (state, cesiumInstance) {
			...
			if(cesiumInstance !== null){
				//state.labels = cesiumInstance.viewer.entities.add(new Cesium.LabelCollection());
				console.log('初始化labels');
			}
        },
    ...
    actions: {
        async setCesiumInstance ({commit}, cesiumInstance) {
            commit('SET_CESIUMINSTANCE', cesiumInstance)
            //debugger;
			console.log(store.state.cesiumInstance)
        }
    },
    getters: {
